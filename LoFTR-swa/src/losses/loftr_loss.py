...
class LoFTRLoss(nn.Module):
    ...
    def compute_coarse_loss(self, conf, conf_gt, weight=None):
        ...
        # Align tensor sizes
        conf, conf_gt, weight = self.align_tensor_size(conf, conf_gt, weight)
    ...
    def align_tensor_size(self, tensor_a, tensor_b, tensor_c=None):
        """ Align tensor sizes by slicing if necessary. """
        _, h_a, w_a = tensor_a.shape
        _, h_b, w_b = tensor_b.shape
        h_min = min(h_a, h_b)
        w_min = min(w_a, w_b)
        ...
        if tensor_c is not None:
            _, h_c, w_c = tensor_c.shape
            h_min = min(h_min, h_c)
            w_min = min(w_min, w_c)
            tensor_a = tensor_a[:, :h_min, :w_min]
            tensor_b = tensor_b[:, :h_min, :w_min]
            tensor_c = tensor_c[:, :h_min, :w_min]
            return tensor_a, tensor_b, tensor_c

        return tensor_a, tensor_b, tensor_c

    def compute_fine_loss(self, expec_f, expec_f_gt):
        if expec_f.numel() == 0 or expec_f_gt.numel() == 0:
