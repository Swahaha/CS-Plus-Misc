...
# from src.utils.misc import get_rank_zero_only_logger, setup_gpus
# from src.utils.profiler import build_profiler
# from src.lightning.data import ImagePairDataModule
# from src.lightning.lightning_loftr import PL_LoFTR
...
#     parser.add_argument('--pin_memory', type=bool, default=True, help='whether loading data to pinned memory or not')
...
#     parser.add_argument('--accelerator', type=str, default='ddp', help='accelerator for distributed training')
#     parser.add_argument('--check_val_every_n_epoch', type=int, default=1, help='validation check frequency')
#     parser.add_argument('--log_every_n_steps', type=int, default=100, help='logging frequency')
#     parser.add_argument('--limit_val_batches', type=float, default=1.0, help='fraction of validation batches to use')
#     parser.add_argument('--num_sanity_val_steps', type=int, default=10, help='number of sanity check validation steps')
...
#     # Load data config
#     data_cfg_file = Path(args.data_cfg_path)
#     data_cfg_file = data_cfg_file.stem.replace(".py", "")
#     data_cfg_module = __import__(f"{data_cfg_file}", fromlist=['cfg'])
#     data_config = data_cfg_module.cfg

#     # Load main config
#     main_cfg_file = Path(args.main_cfg_path)
#     main_cfg_file = main_cfg_file.stem.replace(".py", "")
#     main_cfg_module = __import__(f"{main_cfg_file}", fromlist=['cfg'])
#     main_config = main_cfg_module.cfg

#     config = get_cfg_defaults()
#     config.merge_from_other_cfg(data_config)
#     config.merge_from_other_cfg(main_config)
#     pl.seed_everything(config.TRAINER.SEED)  # reproducibility
...
#     data_module = ImagePairDataModule(args, config)
#     loguru_logger.info(f"LoFTR DataModule initialized!")
